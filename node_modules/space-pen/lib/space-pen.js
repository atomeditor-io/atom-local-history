(function() {
 ...
 source track wbr'.split(/\s+/).forEach(function(tag) {
  ...
  registerElement = function(tagName) {
    var CustomElement, customTagName, elementConstructor, key, _base;
    customTagName = "space-pen-" + tagName;
    if ((_base = window.__spacePenCustomElements)[customTagName] == null) {
      _base[customTagName] = false;
      if (typeof window.customElements !== "undefined" && window.customElements.get(customTagName) == null) {
        elementConstructor = document.createElement(tagName).constructor;
        CustomElement = function() {
          return Reflect.construct(elementConstructor, [], CustomElement);
        };
        CustomElement.prototype = Object.create(elementConstructor.prototype);
        for (key in CustomElementPrototype) {
          if (key === "attachedCallback") {
            CustomElement.prototype.connectedCallback = CustomElementPrototype[key];
          } else if (key === "detachedCallback") {
            CustomElement.prototype.disconnectedCallback = CustomElementPrototype[key];
          } else {
            CustomElement.prototype[key] = CustomElementPrototype[key];
          }
        }
        window.customElements.define(customTagName, CustomElement, {
          "extends": tagName
        });
        _base[customTagName] = true;
      }
    }
